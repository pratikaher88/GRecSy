...
class MovieLens(object):
    ...
    def __init__(self, name, device, mix_cpu_gpu=False,
        ...
        def _npairs(graph):
            ...
            return sum([graph.number_of_edges(et) for et in graph.etypes])
    ...
    def _generate_enc_graph(self, rating_pairs, rating_values, add_support=False):
        ...
        #     rrow = rating_row[index]
        #     rcol = rating_row[index]
            
        #     data_dict.update({
        #         ('user', 'rates', 'movie'): (rrow, rcol),
        #         ('movie', 'rev-rates', 'user'): (rcol, rrow)
        #     })
        ...
        print( len(rating_pairs[0]), [graph.number_of_edges(et) for et in graph.etypes])
